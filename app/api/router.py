...
@router.post("/inlet")
async def process_inlet(request: Request):
    """
    ...
    try:
        ...
        logger.info(f"Original request keys: {list(body.keys())}")
        
        # Process and anonymize the body
        anonymized_body, entities_map = anonymize_data(body)
        ...
        if set(body.keys()) != set(anonymized_body.keys()):
            # Log the difference for debugging
            missing_keys = set(body.keys()) - set(anonymized_body.keys())
            extra_keys = set(anonymized_body.keys()) - set(body.keys())
            if missing_keys:
...
@router.post("/outlet")
async def process_outlet(request: Request):
    """
    ...
    try:
        ...
        logger.info(f"Original response keys: {list(body.keys())}")
        
        # If no mapping ID in body but we have a latest one, add it
        if ("metadata" not in body or "privacy_mapping_id" not in body.get("metadata", {})) and latest_mapping_id:
            logger.info(f"Adding latest mapping ID to response: {latest_mapping_id}")
            if "metadata" not in body:
                body["metadata"] = {}
            body["metadata"]["privacy_mapping_id"] = latest_mapping_id
        
        # Process and deanonymize the body
        deanonymized_body = deanonymize_data(body)
        
        # Log keys in the deanonymized response for debugging
        logger.info(f"Deanonymized response keys: {list(deanonymized_body.keys())}")
        
        # Verify the structure remains consistent
        if set(body.keys()) != set(deanonymized_body.keys()):
            # Log the difference for debugging
            missing_keys = set(body.keys()) - set(deanonymized_body.keys())
            extra_keys = set(deanonymized_body.keys()) - set(body.keys())
            if missing_keys:
            ...
            for key in body.keys():
                if key not in deanonymized_body and key != "metadata":
                    deanonymized_body[key] = body[key]
                    logger.info(f"Restored missing key: {key}")
